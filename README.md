...
在 Android 16 (ColorOS 16) 的时代，系统对后台应用的管理变得极其霸道。即使你手动将应用（如“小米运动健康”）的电池优化设置为“无限制”，系统依然会在重启、切换网络、或者云控下发指令时，悄悄把它改回“优化”并强行冻结，导致手表断连、消息收不到。

本模块专为解决这一痛点而生。它不仅在内存中拦截系统底层的篡改行为，还结合了 Root 脚本与 CI/CD 流水线，形成了一套**“三位一体”的终极防御体系**，彻底把选择权还给用户。

## ✨ 核心功能

*   **内存拦截 (Xposed Hook)**：精准切入 `system_server`，反编译并拦截 `OplusHansManager` 与 `OplusDeviceIdleHelper` 的篡改逻辑，伪造顶级白名单豁免。
*   **内存守护线程**：在系统框架内部开启守护线程，每 60 秒主动调用系统命令，实时纠正底层状态。
*   **开机自启脚本 (Root)**：通过 UI 自动向 `/data/adb/service.d/` 写入守护脚本，开机后以最高 Root 权限无限循环执行强制纠正。
*   **动态 UI 管理**：内置极简列表，用户可自行搜索并勾选需要保护的应用，配置实时写入底层。
*   **持久化日志**：绕过系统日志拦截，所有操作直接写入 `/data/system/BatteryWhitelist.log`，排查状态一目了然。
*   **固定签名与 CI/CD**：支持 GitHub Actions 云端编译，打 Tag 自动构建并发布带有固定签名的 Release 版本。
...
1. 前往 Releases 页面下载最新版本的 BatteryWhitelist-release.apk。
2. 在手机上安装该 APK。
...
6. 在 LSPosed 管理器中启用本模块，作用域只勾选「Android 系统（android）」。
7. 重启手机。
...
本项目采用 GitHub Actions 进行全自动云端构建，无需本地配置复杂的 Android 环境。

持续集成 (CI)

· 触发条件：推送到 main / master 分支，或提交 PR。
· 产物：BatteryWhitelist-debug-*.apk（保留 7 天）。

发布流水线 (Release)

· 触发条件：推送版本标签（如 git tag v1.2.0 && git push origin v1.2.0）。
· 流程：自动使用固定签名打包，生成 Release APK，并自动创建 GitHub Release。
· 产物：BatteryWhitelist-release-*.apk（永久保留在 Release 页面）。
